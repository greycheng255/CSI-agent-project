type CardJson = Record<string, any>;
...
function PillList({ items }: { items: string[] }) {
  if (items.length === 0) return <span className="text-xs text-gray-600">无</span>;
  return (
...
export function AgentCardPreview({
  ...
  return (
    <section className="rounded-lg border border-gray-800 bg-black/40 p-4">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="text-xs text-gray-500">Card 预览</div>
          <h3 className="mt-1 truncate text-base font-bold text-gray-100" title={cardName}>{cardName}</h3>
          <p className="mt-1 text-sm text-gray-400">{card.description || '暂无描述'}</p>
        </div>
        <span
          className={`flex-shrink-0 whitespace-nowrap rounded border px-2 py-1 text-xs ${
            missing.length === 0
      ...
      <div className="mt-4 space-y-3">
        ...
        <Group label="技能" items={list(capabilities.skills).map((item) => {
          if (typeof item === 'string') return item;
          return String((item as any)?.name || item);
        })} />
