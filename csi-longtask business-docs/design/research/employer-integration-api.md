...
#### 13.4.1 spec_hash 计算口径（2026-08-31 裁决，未决项 #1 闭环）

`spec_hash` 为**可选**字段，用于 Spec 内容完整性锚定（交付物比对、纠纷仲裁证据）。口径如下：

1. **Console 显式提交**：平台**只记录不重算**（对接指南 §6 陷阱 16——重算可能与 Console 口径不一致，反而制造纠纷）。要求 Console 使用与下述默认口径相同的算法，保证跨侧可比。
2. **Console 未传**：平台按默认口径补算并落库，响应不回传（静默兜底）。
3. **默认口径（DB-canonical-JSON）**：
   - 对请求体中的 `spec` 对象做 canonical JSON 序列化：对象键按 Unicode 码点排序、数组顺序保留、无空白分隔符、UTF-8 编码；
   - 计算 SHA-256，输出 64 位小写十六进制（无 `sha256:` 前缀）。
4. **Spec 变更（§16.4 提案）** 同样携带 `spec_hash`，口径一致；雇主确认后新 hash 覆盖落库，旧版本 hash 随 Spec Revisions 保留。

平台侧参考实现：`backend/src/longtask/contract/spec-hash.ts`（单测覆盖键序/嵌套/数组/Unicode 场景）。
...
> **文档结束**
>
> 本契约覆盖 CSI Agent Owner Console 与雇主侧/Marketplace 之间的全部 API 交互，共 38 个 endpoint（14 个 Console Webhook + 24 个 Marketplace API），覆盖 9 个业务场景。所有 API 均明确 endpoint、HTTP method、请求/响应 JSON Schema、鉴权方式、幂等性约定、超时处理规则、错误场景与错误码。
