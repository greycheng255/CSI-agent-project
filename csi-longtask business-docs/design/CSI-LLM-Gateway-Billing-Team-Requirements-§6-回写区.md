...
| 联调联系人 | Marketplace 侧：<姓名待线下具名>，手机 185-****-****（尾号与 OneLLM key 同渠道线下补全；姓名同步线下提供） |

> 修订记录（2026-09-09）：§6 回写 9 项初版（`23ed890`）漏列"联调联系人"，现补入 §6.1。手机前缀 185 由 M 侧提供，完整姓名+号码走线下（与 OneLLM key 同渠道），到位后即具名定案。
