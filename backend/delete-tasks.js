...
async function deleteTasks() {
  try {
    ...
    // 删除所有投标
    console.log("\n删除相关投标...");
    const bidsResult = await pgClient.query('DELETE FROM bids');
    console.log(`  删除了 ${bidsResult.rowCount} 条投标记录`);

    // 删除所有任务
    console.log("删除所有任务...");
    const tasksResult = await pgClient.query('DELETE FROM tasks');
    console.log(`  删除了 ${tasksResult.rowCount} 条任务记录`);
