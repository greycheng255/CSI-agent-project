...
function createHiClawWorkflowFixture() {
  ...
  const ordersService = {
    ...
    deliver: jest.fn(async (orderId: string, ownerUserId: string, data: any) => {
      state.delivery = {
        id: 'delivery-001',
        orderId,
        ownerUserId,
  ...
  ordersRepository.find.mockImplementation(async () => (state.order ? [state.order] : []));
  deliveriesRepository.findOne.mockImplementation(async () =>
    state.delivery?.status === DeliveryStatus.REJECTED ? state.delivery : null,
  );
...
describe('MCPToolsProvider HiClaw workflow', () => {
  it('supports quote, accepted task, progress, delivery, revision, and completion', async () => {
    ...
    const listOpen = await fx.tools.get('platform.task.list_open')!.execute(
      { skills: ['react'], page: 1, pageSize: 20 },
      fx.ctx,
    );
    expect((listOpen.data as any).tasks[0].taskId).toBe(fx.task.id);

    const detail = await fx.tools.get('platform.task.get')!.execute(
      { taskId: fx.task.id },
      fx.ctx,
    );
    expect((detail.data as any).attachments[0].url).toContain('requirements.pdf');
    ...
    const duplicate = await fx.tools.get('platform.quote.submit')!.execute(
      { taskId: fx.task.id, priceCny: 150 },
      fx.ctx,
    );
    expect(duplicate.success).toBe(false);
    expect(duplicate.error?.code).toBe('DUPLICATE_BID');

    const pendingBid = await fx.tools.get('platform.quote.get_my')!.execute(
      { taskId: fx.task.id },
      fx.ctx,
    );
    ...
    const acceptedBid = await fx.tools.get('platform.quote.get_my')!.execute(
      { taskId: fx.task.id },
      fx.ctx,
    );
    ...
    const myTasks = await fx.tools.get('platform.order.list_my')!.execute({}, fx.ctx);
    expect((myTasks.data as any).tasks[0].status).toBe('IN_PROGRESS');

    const progress = await fx.tools.get('platform.order.update_execution')!.execute(
      {
        taskId: fx.task.id,
        phase: '核心开发',
        progress: 65,
    ...
    const waiting = await fx.tools.get('platform.task.get_status')!.execute(
      { taskId: fx.task.id },
      fx.ctx,
    );
    ...
    const revision = await fx.tools.get('platform.task.get_status')!.execute(
      { taskId: fx.task.id },
      fx.ctx,
    );
    ...
    const completed = await fx.tools.get('platform.task.get_status')!.execute(
      { taskId: fx.task.id },
      fx.ctx,
    );
